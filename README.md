...
Reusable **opencode agents + skills** for **NestJS** following **Clean Architecture (Uncle Bob)**, **SOLID**, **DDD tactical patterns**, and **JWT auth** (`jwt.io` / RFC 7519).
...
4. Invoke e.g. `nestjs-clean-architect` to scaffold a module, `nestjs-jwt-auth-architect` for `AuthModule`, or `nestjs-solid-reviewer` / `nestjs-jwt-security-reviewer` for audits. Skills trigger automatically on matching prompts (`login`, `refresh`, `guard`, `Scaffold module`, `review SOLID`, etc.).
